  ...
  (let ((runner (require-runner-from-ctx ctx)))
    (postmodern:with-connection *db-spec*
      (let* ((run-id (slot-value request 'cave::run-id))
        ...
        ;; Authorization: a runner may only register artifacts for a workflow run
        ;; it was actually assigned a job of. Without this gate any authenticated
        ;; runner could attach an attacker-chosen object-path to any run, which
        ;; the download route then reads from the object store — an arbitrary
        ;; host-file read when combined with a traversing path.
        (unless (runner-owns-run-p (getf runner :id) run-id)
          (error "runner is not assigned to this run"))
        (let ((row (when (and (plusp run-id) (plusp (length name)) (plusp (length object-path)))
